(function() {
  ...
  const terminal = {
    ...
    isFieldsLocked() {
      return !state.officerId || state.officerId.trim() === '' || 
             !state.shiftCode || state.shiftCode.trim() === '' || 
  ...
  const templates = {
    async loadList() {
      try {
        ...
        const processedTemplates = await Promise.all(txtFiles.map(async (file) => {
          try {
  ...
  function updateFieldCounter() {
    ...
    if (state.currentTemplate && state.currentTemplate.fields) {
      filled = state.currentTemplate.fields.filter(f => f.value.trim() !== '').length;
    }
  ...
  function generateOutput() {
    ...
    state.currentTemplate.fields.forEach((field) => {
      ...
      rawResult = rawResult.replace('[field]', value);
      htmlResult = htmlResult.replace(
        '[field]', 
        value ? `<span class="understood">${value}</span>` : '<span class="paper_field"></span>'
      );
    });
    ...
    if (dom.outputTerminal && dom.outputTerminal.dataset.userEdited !== 'true') {
  ...
  function pencodeToHtml(text) {
    ...
    const tagPattern = new RegExp(Object.keys(tagMap).map(key => 
      key.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
    ).join('|'), 'g');
    ...
    t = t.replace(/\[redacted\](.*?)\[\/redacted\]/gs, (match, content) => 
    ...
    t = t.replace(/\[color=([^\]]+)\](.*?)\[\/color\]/gs, '<span style="color: $1;">$2</span>');
    t = t.replace(/\[lang=([^\]]+)\](.*?)\[\/lang\]/gs, '<span class="language" data-lang="$1" title="Language: $1">$2</span>');
  ...
  function getCurrentOutput() {
    return dom.outputTerminal ? 
      utils.getTextContent(dom.outputTerminal) : 
      state.currentRaw;
  }
  ...
  async function initialize() {
    const elementIds = [
      ...
      'systemTime', 'officerInput', 'shiftInput'
    ];
